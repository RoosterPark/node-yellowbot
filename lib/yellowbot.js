...
api.configure = function (params) {
    ...
    if (api.debug) {
      console.log("CONFIG", JSON.stringify(this, null, "  "));
    }
};
...
api.get = function(method, params, cb) {
    ...
    var path = this.base_path + method + '?' + querystring.stringify(params);

    if (api.debug) { console.log("GET:", this.server.host, path); }
...
api.post = function(method, params, data, cb) {
    ...
    var req = this.client.request(
        _.extend({},
            ...
            {
                method: 'POST',
                path: this.base_path + method + '?' + querystring.stringify(params),
                headers: _.extend({},
                    this.server.headers,
                    {
...
api.post_form = function(method, params, cb) {
    ...
    var url = this.protocol + "://" + this.server.host + this.base_path + method;
